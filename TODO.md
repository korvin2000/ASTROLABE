...
## 1 Progress
- **Phase:** P1 · **Next task:** P1.8.2 · **Blocked:** none
...
- **Completion state (end of session 1, 2026-09-20):** P0 complete (26 tasks `DONE`: `IMPLEMENTED`, and `FIXTURE_VALIDATED` on Windows only — Linux runs only through the CI workflow, which has not yet executed on a remote). P1 `DONE`: P1.1.1–P1.1.4, P1.2.1–P1.2.6, P1.3.1–P1.3.4, P1.4.1–P1.4.4, P1.5.1–P1.5.3, P1.6.1–P1.6.10, P1.7.1–P1.7.8, P1.8.1, P1.10.1–P1.10.4. Nothing is `IN_PROGRESS`; the working tree is clean at the commit that carries this line.
...
- **Resume notes**
  - Build: `export JAVA_HOME=/c/Users/user/.gradle/jdks/eclipse_adoptium-26-amd64-windows.2` (Temurin 26.0.2.1; Windows form `C:\Users\user\.gradle\jdks\eclipse_adoptium-26-amd64-windows.2`), then `./gradlew build` (≈ 2 min: 675 tests, 6 platform-gated skips on Windows). Targeted runs: `./gradlew :core:test --tests 'io.astrolabe.<pkg>.*' --console=plain`. Kotlin ABI validation is on: after any public-API change run `./gradlew :core:updateKotlinAbi` (and `:provider-api:updateKotlinAbi` when touched) as its own invocation, then `build`, and commit the `*/api/*.api` dumps. The test JVM passes `--enable-native-access=ALL-UNNAMED` (FFM in `os`).
  - Machine facts: git 2.45, ripgrep 15.2, Python 3.14 (no pytest ⇒ the unittest path is the exercised one), Node 24, no WSL distro, no Docker ⇒ every POSIX code path (process groups, symlinks, directory fsync) stays unverified until CI runs on `ubuntu-latest`.
  - Working method: contracts and the cell/controller spine were written in the main tree; self-contained packages were delegated to parallel worktree agents (`.claude/worktrees/agent-*`, branches `worktree-agent-*`), merged with `--no-ff`, ABI dumps regenerated after each merge, worktrees removed with PowerShell `Remove-Item -LiteralPath '\?\<path>'` (long paths). Each merged package's report is condensed in its task `Log:` line, including what stays unverified.
  - Integration debts owned by upcoming tasks: `[S]` must render `Boundary.DATA_RULE` + `ExecutionModeLabel` (P1.8.2); look/recall/export must route bytes through `Redaction` and register `RedactionMask`s (P1.6.3); the runner must take `RedactionConfig.envAllowlist` as `EnvPolicy.inheritedNames`, capture with `--no-color`/`CI=1`, and journal `Preimages` records (P1.6.4/P1.6.5); campaign open maps `Sniff.PackageCommands` → `RunnerCommands` for `Checks.seed` and `RulesTrust.approved` → `Prime.RulesSnapshot` (P1.1.2/P1.9.2); `VersionRegistry.show` returns coverage `Ranges` and the look tool renders the bytes; the generic shaper reports `Inconclusive` for ruff/tsc/eslint until the P3.1.4 parsers exist; mutators announce every transition through `VersionRegistry.change(path, from, to, cause)` — the edit tool with its CAS `expect` version, the runner from the stamp diff with `from = null` when nobody read the file (P1.6.4/P1.6.5); the cell wires `Coherence.register` for Workset → register holder → `Checks` in that order, drains `Coherence.takeScheduled()` at the end-of-turn boundary for the checker and `Atlas.refresh`, and calls `Checks.refresh(stampNow)` after every stamp (P1.7.2/P1.7.4/P1.8.7); campaign open binds the workspace's `ProtectedPaths` to the committed contract's protected list and calls `Contracts.open` on `deriveS0`'s result after the workspace is captured (P1.9.2); the dispatcher runs `ScopeGuard.check` against `Contracts.current` before any edit and `TestIntegrity.baseline` over every touched path, edit or run (P1.6.2/P1.6.4/P1.6.5); the exit gate consumes `TestIntegrity.unresolved` (P1.7.7).
