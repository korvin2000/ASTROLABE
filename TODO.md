...
## 1 Progress
- **Current: P1.8.2 Layout, P1.8.3 Anchor and P1.8.4 Gauge DONE (2026-09-21); P0 validation repaired and in CI.** The cell's rendering half is complete: `Layout` for the cached `[S][R][K][T]` prefix, `Anchor` for the volatile `[A]` tail with per-block caps and a measured size, `Gauges` for the ~20-token line on every result. 20 focused tests. All four failures of CI run 35514932596 — repeated exactly by run 35532489501, so there is no fifth mode — were diagnosed from those runs' own logs and artifacts and repaired, one a real product defect in `Git`'s D-53 index guard; ripgrep and Node 24 are pinned in the workflow. Run 35621166065 reached both suites for the first time: Windows **878 tests, 1 failure, 2 skips** (was 758/3/130) and Linux **878, 1, 3** (was: never started). The single remaining failure on both was a host-dependent status assertion this session had added to FX-22, now removed; run 35622134003 verifies that. Local Windows/JDK 26 full build: core **898/0 failures/errors/6 existing platform skips**, eval 30, provider-api 15; ABI additive. **72/185 DONE, 4 IN_PROGRESS, 109 TODO**. P0.1.2, P0.6.1, P0.6.2 and P0.6.4 stay IN_PROGRESS until both remote jobs are green. Next P1 code task: **P1.8.5 Gates**.
...
- **Current phase counts:** P0 15 DONE + 4 IN_PROGRESS; P1 48 DONE + 16 TODO; P2-P6 9 DONE + 93 TODO. Counts derive from unique task headings, not code volume.
- **Phase:** P0 validation repaired and in CI; P1 cell runtime under way · **Next task:** **P1.8.5 Gates** · **Also open:** P0.1.2 (with P0.6.1/P0.6.2/P0.6.4), whose remaining work is a green run on both platforms · **Owner decisions blocking work:** none
...
- **Remaining P1 in dependency order:** P1.8.5–P1.8.8 cell runtime (Gates → Residency → Cell loop → ResultPacket) → P1.9.1–P1.9.6 controller/facade → P1.11.1–P1.11.2 telemetry → P1.12.1–P1.12.4 validation (the `Deps` fields stay authoritative).
- **Resume notes**
  ...
  - **Remaining runtime integration:** P1.8.2 renders Boundary.DATA_RULE + ExecutionModeLabel; P1.9.2 opens the derived contract after workspace capture, binds ProtectedPaths, passes Sniff commands into Checks.seed and approved rules into Prime; P1.8.7 wires Coherence holders in Workset -> register -> Checks order, drains scheduled checker/Atlas refreshes, refreshes checks after stamps, admits work through CellBudget and passes unresolved integrity flags to the exit gate. Run-side TestIntegrity collection and preimage journaling remain unwired (no corresponding calls in Run/Runner); resolve them when assembling the tools/runtime rather than assuming component DONE means integrated. Generic shapers still return Inconclusive for ruff/tsc/eslint until P3.1.4.
...
Local verification (Windows / JDK 26 / Gradle 9.7.1, offline): full `build` — core **878 tests, 0 failures/errors, 6 existing platform skips**, eval **30**, provider-api **15**; ABI dumps unchanged (the changes are private or in `testFixtures`). **This does not close P0.1.2**: acceptance is both remote jobs green, and Linux is still entirely unvalidated.
...
#### P0.1.2 [V] CI matrix · IN_PROGRESS
...
#### P0.6.1 [C][M] `Os` and `Proc` · IN_PROGRESS
...
#### P0.6.2 [C][M] `Git` CLI wrapper · IN_PROGRESS
