...
## 1 Progress
- **Current: P0 validation repair applied (2026-09-21), awaiting the remote CI run.** All four recorded failures of run 35514932596 are diagnosed from that run's own evidence and repaired, one of them a real product defect in `Git` (D-53 index guard); ripgrep and Node 24 are pinned in the workflow so P0.6.3's and P0.6.4's criteria are actually exercised on CI. See [§1.1](#11-targeted-readiness-audit-2026-09-20) for the evidence table and the four task logs for the repairs. Local Windows/JDK 26 full build: core **878/0 failures/errors/6 existing platform skips**, eval 30, provider-api 15; ABI unchanged. P0.1.2, P0.6.1, P0.6.2 and P0.6.4 stay **IN_PROGRESS** until both remote jobs are green — local passes do not close them, and Linux remains unvalidated. Next P1 code task unchanged: **P1.8.2**.
...
- **Phase:** P0 validation reopened, repairs applied and awaiting CI; P1 implementation checkpoint retained · **Next task:** P0.1.2 (with P0.6.1/P0.6.2/P0.6.4) — remaining work is the remote run, which needs a push · **Next P1 code task:** P1.8.2 · **Owner decisions blocking work:** none
