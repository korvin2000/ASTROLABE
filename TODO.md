...
## 1 Progress
- **Current: P1.8.2 Layout DONE (2026-09-21); P0 validation repaired and in its second CI run.** `cell.Layout` renders the cached `[S][R][K][T]` regions with byte-stable inputs and a fake-adapter-accepted request (7 tests). All four failures of CI run 35514932596 — which run 35532489501 at `3f3edc4` repeated exactly, so there is no fifth mode — were diagnosed from those runs' own logs and artifacts and repaired, one of them a real product defect in `Git`'s D-53 index guard; ripgrep and Node 24 are pinned in the workflow. Run 35621166065 is the first to reach the Linux suite: **878 tests, 1 failure, 3 skips** (was: never started, and 130 Windows skips). That failure was a host-dependent status assertion this session had just added to FX-22, now removed. Local Windows/JDK 26 full build: core **885/0 failures/errors/6 existing platform skips**, eval 30, provider-api 15; ABI additive. **70/185 DONE, 4 IN_PROGRESS, 111 TODO**. P0.1.2, P0.6.1, P0.6.2 and P0.6.4 stay IN_PROGRESS until both remote jobs are green. Next P1 code task: **P1.8.3 Anchor**.
...
- **Current phase counts:** P0 15 DONE + 4 IN_PROGRESS; P1 46 DONE + 18 TODO; P2-P6 9 DONE + 93 TODO. Counts derive from unique task headings, not code volume.
- **Phase:** P0 validation repaired and in CI; P1 cell runtime under way · **Next task:** **P1.8.3 Anchor** · **Also open:** P0.1.2 (with P0.6.1/P0.6.2/P0.6.4), whose remaining work is a green run on both platforms · **Owner decisions blocking work:** none
...
- **Remaining P1 in dependency order:** P1.8.3–P1.8.8 cell runtime (Anchor → Gauge → Gates → Residency → Cell loop → ResultPacket) → P1.9.1–P1.9.6 controller/facade → P1.11.1–P1.11.2 telemetry → P1.12.1–P1.12.4 validation (the `Deps` fields stay authoritative).
- **Resume notes**
