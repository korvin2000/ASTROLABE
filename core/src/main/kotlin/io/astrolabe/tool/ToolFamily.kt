...
public object ToolOps {
     ...
     * bmap and kb.propose arrive later; propose is S1+, delegate/collect S2+ through the shape mask, P4.4.1). `verify.review` is the campaign-scope human review path (P3.5.2, D-23); the review cell is P4.4.3.
     */
    public val implementingS0: ToolMask = ToolMask(
        all - setOf("look.bmap", "task.delegate", "task.collect", "task.propose", "kb.propose"),
    )
}
