...
public class Controller @JvmOverloads public constructor(
    ...
    private suspend fun plan(c: OpenedCampaign, model: CellModel, authority: Authority, syntax: SyntaxCheck, span: SpanId?, packets: MutableList<ResultPacket>): Transition.Stopped? {
        ...
        val compiler = Compiler(model.estimator, c.attempt.config)
        val routing = route(c, RoutingFunction.Plan, planning, model, null, compiler.compile(planning, contract, model.profile, Roles.plan, c.prime, maxOutputTokens = model.maxOutputTokens)) { profile ->
            compiler.compile(planning, contract, profile, Roles.plan, c.prime, maxOutputTokens = model.maxOutputTokens)
        }
        val compiled = routing.compiled
        ...
        val run = runCell(c, cellId, planning, Roles.plan, routing.model, authority, syntax, compiled, span, null, proposals = intake, completion = completion)
    ...
    private suspend fun runS0(campaign: OpenedCampaign, model: CellModel, authority: Authority, syntax: SyntaxCheck, span: SpanId?): S0Run {
        ...
        val increment = dispatched.graph.increments.first { it.id == ready.id }
        val register = carry?.register?.copy(cell = cellId, increment = increment.id, incrementTitle = increment.title)
        val run = runCell(c, cellId, increment, Roles.implementing, cellModel, authority, syntax, compiled, span, dispatched.ledger, register, seeds?.shown.orEmpty(), pinned = listOfNotNull(resume))
        routing.selected?.let { router.record(it, outcomeOf(run.exit)) }
        val ids = run.ids
     ...
     * a fake-profile campaign routes to the profile it was given (D-130). The risk floor reads the increment's (else
     * the contract's) declared risk and the open-time impact pre-scan; tokens are admitted by the cell itself (D-06),
     * so only a monetary budget caps affordability here (D-131). A routed profile other than the supplied one recompiles.
     */
